...
int main(int argc, char* argv[]) {
    ...
    // This logic still only processes the first ticker in the config file.
    const auto& cfg = cfgs[0];
    std::string csv_file = cfg.ticker + ".csv";
    std::vector<Candle> candles = readData(csv_file);

    if (candles.size() < 21) { // Need 21 for 20-period SMA and prior candle
        std::cerr << "Not enough data to run strategy. Need at least 21 candles." << std::endl;
        return 1;
    }
    ...
    double rsi14 = computeRSI(closes, 14); // Common RSI period is 14

    std::string signal = "HOLD";
    double entry = candles.back().close;
    double sl = 0, tp = 0;

    // --- NEW STRATEGY LOGIC: SMA Crossover with RSI Confirmation ---
    if(sma5 > sma20 && rsi14 > 50 && sma5 > 0 && sma20 > 0) {
        signal = "BUY";
        sl = entry - 1.5 * atr14;
        tp = entry + 2.0 * atr14;
    } else if(sma5 < sma20 && rsi14 < 50 && sma5 > 0 && sma20 > 0) {
        signal = "SELL";
        sl = entry + 1.5 * atr14;
        tp = entry - 2.0 * atr14;
    }
    ...
    std::cout << " | SMA5=" << sma5 << " SMA20=" << sma20 << " ATR14=" << atr14 << " RSI14=" << rsi14 << "\n";
