...
MAX_ROWS = 7000 # Set max rows to keep the data file lean
...
with open("conf.txt") as f:
    for line in f:
        ...
        pair = line.split()[0] # Read only the pair
        csv_file = f"{pair}.csv"

        print(f"Updating data for {pair} (5m)")

        # Fetch enough data to calculate ATR correctly (e.g., 30 days for a 14-period ATR)
        new_data_df = yf.download(
            tickers=pair,
            interval="5m",
            period="30d", #FOR FIRST RUN
            #period="3d",
            progress=False,
            auto_adjust=False
        )
        ...
        # Drop initial rows where ATR could not be calculated
        ...
        # Trim old data if file is too large
